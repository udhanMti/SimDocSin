...
def doc_to_sentence_t(doc, lang):
    s = []
    if lang == 'en':
...
def doc_to_sentence_w(doc, lang):
    s = []
    if lang == 'en':
        s = splitter.split(text=doc)
    elif lang == 'si':
...
def doc_to_sentence3(doc):
    # assume . is only fullstop and all first sentence has fullstop
    dd = doc.split('.')[:-1]
    if len(dd) == 0:
        doc = doc+'.'
        return doc.split('.')[:-1]
...
def doc_to_sentence(doc, lang):
    sentences = doc_to_sentence_t(doc,lang)
...
def doc_to_sentence1(doc, lang):
    sentences = doc_to_sentence_w(doc, lang)
    min_tokens = 2 if lang == 'en' else 2
    max_tokens = 10 if lang == 'si' else 10
    total_sent = len(sentences)
