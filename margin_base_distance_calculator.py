...
def margin_base_score(source_docs, target_docs, source_weight, target_weight, target_lang):
    if (target_lang == 'si'):
        f = 1024
...
def margin_base_score_np(source_document, source_weights, target_lang):
    if (target_lang == 'si'):
        f = 1024
        u = AnnoyIndex(f, 'euclidean')
        u.load('../MassDoc/test.ann')
        ...
        cmap_file = open('../MassDoc/sent_count_map.json', encoding='utf8')
        ...
        cmap_file = open('../MassDoc/sent_count_map_en.json', encoding='utf8')
        cmaps = json.load(cmap_file)
    ...
    for i in range(len(source_document)):
        ...
        for j in all_docs:
            target_weight = np.load(
                '../MassDoc/army/' + str(j // 1000) + '/w' + target_lang + '/' + str(j % 1000) + '.npy')
            ...
            # target_docs = np.load('../MassDoc/army/' + str(j // 1000) + '/'+target_lang+'/' + str(j % 1000) + '.npy')
